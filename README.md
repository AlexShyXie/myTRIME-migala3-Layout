# 我的Trime（同文）输入法库 flypy932.schema
我是用小鹤音形，目前在尝试米旮旯3
...
添加雾凇拼音词库后，我发现日常打字**80%不需要用到3键**，只要2键就行，但是要有意识的打最常见的一些词组，或者你输入的多的3-5字的短语。所以在保持**双拼节奏感和少击键**的同时，完全能胜任日常打字聊天需要。

多数时候，打两字词语的时候，如果不是自己想要的候选，可以**立马跟后一个字的第3码**，
比如说打字`好好学习`，我先打`hchc`，是`4242`，发现候选栏里完全没有我想要的`好好`，那就后面再打一个5键（`好`的单字模式是`425`），就大概率能把`好好`顶到第1排候选里。如果还不行，那就需要用键盘上的移动按钮对候选进行修改了，我设置了移动到首字、末字，单纯左移一下，右移一下，右移两下等相关按钮（因为我是单手，键盘上有很多多余的按钮用不到，所以设置这么多操作方式）
由于开了自动造词功能，所以第1次没有想要的候选词，第2次再打`hchc`，`好好`一定会出现在最前面了。

> 同文输入法设置里一定要把 `虚拟键盘-悬浮窗编码区使用插入符号(^)`打开，这样才能让我们定位修改。

**由于2码3码混输，有时候会存在编码区混乱的情况，需要自己对自己的输入心中有数。**

由于确实九宫格存在很多重码问题，使用这个方法之后最大的问题是单纯2码输入的时候，部分按键组合的动态调频非常明显，但是经过习惯之后，部分输入，可以提前进行3码输入，当然，这种情况也不算多。
